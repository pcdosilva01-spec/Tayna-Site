...
export default function CadastroPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!terms) return toast.error("Você deve aceitar os termos");
    
    setLoading(true);
    try {
      const res = await registerUser({ name, email, password, terms });
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h2 className="text-xl font-heading font-semibold">Criar Conta</h2>
          <p className="text-sm text-muted-foreground mt-1">Faça parte da nossa comunidade</p>
        </div>
        ...
        <div className="p-6 sm:p-8 bg-card border border-border rounded-3xl">
          <form className="space-y-4" onSubmit={handleRegister}>
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Seu nome" required value={name} onChange={e => setName(e.target.value)} disabled={loading}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} placeholder="Mínimo 8 caracteres" required minLength={8} value={password} onChange={e => setPassword(e.target.value)} disabled={loading}
            ...
            <label className="flex items-start gap-2 text-xs text-muted-foreground cursor-pointer">
              <input type="checkbox" checked={terms} onChange={e => setTerms(e.target.checked)} className="w-4 h-4 rounded border-border accent-brand mt-0.5" />
              <span>Concordo com os <Link href="/termos" className="text-brand hover:underline">Termos de Uso</Link> e <Link href="/politica-de-privacidade" className="text-brand hover:underline">Política de Privacidade</Link></span>
            </label>
          ...
          <button onClick={handleGoogleRegister} disabled={loading} className="w-full flex items-center justify-center gap-3 py-3.5 border border-border rounded-xl text-sm font-medium hover:bg-secondary transition-colors disabled:opacity-50">
        ...
        <p className="text-center text-sm text-muted-foreground mt-6">
          Já tem conta? <Link href="/conta" className="text-brand font-medium hover:underline">Entrar</Link>
        </p>
      </motion.div>
    </div>
  );
